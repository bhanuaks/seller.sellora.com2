...
export async function POST(request) { 
    ...
    try{
        ...
        if(childcategory_id){ 
             childcategory = await  ChildCategory.findById(childcategory_id).select("_id childCategoryName slug")
        }
        let brand = null
        let seller = null
        if(brand_id){ 
             brand = await brandModel.findById(brand_id)
        }

        if(seller_id){ 
             seller = await sellerModel.findById(seller_id)
       }
