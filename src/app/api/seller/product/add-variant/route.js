...
const product_medium_img_paths = [
  variant_medium_img_path2,
  variant_medium_img_path3,
  variant_medium_img_path4,
  variant_medium_img_path5,
  ...
  for (let i = 0; i < product_large_img_paths.length; i++) {
    ...
    if (exisPath) {
      const res = await moveFile(
        largPath,
        imageFile,
        `public/${imagePaths[imageIndex - 1].large}`
      );
      console.log(
        largPath,
        imageFile,
      ...
      await moveFile(
        mediumPath,
        imageFile,
        `public/${imagePaths[imageIndex - 1].medium}`
      );
      await moveFile(
        thumbPath,
        imageFile,
        `public/${imagePaths[imageIndex - 1].thumb}`
      );
    }
  }
}
...
const uploadImages = async (image, index, productName) => {
  ...
  for (const [sizeType, size] of Object.entries(imageSizeMap)) {
    await uploadImageFun(
      image,
      `public/${imagePaths[index - 1][sizeType]}`,
      filename,
      size
    );
  }
...
const processImageUpload = async (
  ...
  if (image && typeof image !== "string") {
    ...
    if (
      existProduct &&
      existProduct[`image_${index}`] &&
      existProduct &&
      existProduct[`image_${index}`] != image
    ) {
      await deleteOldImages(index, existProduct, requestImage);
      const oldPath = existProduct[`image_${index}`];
    }
...
export async function POST(request) {
  ...
  if (isEmpty(consumerSalePrice))
    errors.consumerSalePrice = `this field is required.`;
  if (isEmpty(businessSalePrice))
    errors.businessSalePrice = `this field is required.`;
  ...
  if (Object.keys(errors).length > 0) {
    return responseFun(
      false,
      { errors, status_code: 400, image_1: !image_1 },
      200
    );
  }
  ...
  try {
    ...
    if (_id) {
      const sameVariant = await productVariantModel.countDocuments({
        _id: { $ne: new mongoose.Types.ObjectId(_id) },
        sku: sku,
        seller_id: new mongoose.Types.ObjectId(seller_id),
      });
      if (sameVariant > 0) {
        errors.sku = `This sku is already exists.`;
    ...
    if (withImage == "Yes") {
      ...
      const fileShortenedProductName =
        sku.length > 6
          ? sku.slice(0, 6).toString().trim()
          : sku.toString().trim();
      image_1_path = await processImageUpload(
        image_1,
        1,
        existVariant,
        fileShortenedProductName,
        requestImage
      );
      image_2_path = await processImageUpload(
        image_2,
        2,
        existVariant,
        fileShortenedProductName,
      ...
      image_3_path = await processImageUpload(
        image_3,
        3,
        existVariant,
        fileShortenedProductName,
      ...
      image_4_path = await processImageUpload(
        image_4,
        4,
        existVariant,
        fileShortenedProductName,
      ...
      image_5_path = await processImageUpload(
        image_5,
        5,
        existVariant,
        fileShortenedProductName,
      ...
      image_6_path = await processImageUpload(
        image_6,
        6,
        existVariant,
        fileShortenedProductName,
      ...
      image_7_path = await processImageUpload(
        image_7,
        7,
        existVariant,
        fileShortenedProductName,
    ...
    // ========================image uploading end ===============================================================
    const CategotyVariant = await Category.findById(category_id).select(
      "category_variant"
    );

    if (
      variant == "No" ||
      Object.keys(CategotyVariant?.category_variant).length === 0
    ) {
    ...
    const thresholdArr = threshold ? JSON.parse(threshold) : [];
    ...
    return responseFun(
      true,
      { message: "Variant Addedd", variantList: variantList },
      200
    );
  } catch (error) {
    console.log(error);
  ...
  for (const path of variantPaths) {
    if (await checkFilePath(`public/${path}`, image)) {
      await Promise.all([
        ...
        moveFile(
          `public/${path.replace("large", "medium")}`,
          image,
          `public/${variant_medium_img_path1}`
        ),
        moveFile(
          `public/${path.replace("large", "thumb")}`,
          image,
          `public/${variant_thumb_img_path1}`
        ),
      ]);
    }
  }
...
export async function newSinNumber() {
  ...
  try {
    const existAlready = await productVariantModel.countDocuments({
      sin: sinNumber,
    });
    if (existAlready > 0) {
      newSinNumber();
